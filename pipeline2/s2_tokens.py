...
TWO CORRECTIONS TO THE WRITTEN DESIGN, both declared before the run:
  ...
  D-26  The R2 baseline is the training MEAN, not the median. Under squared error the mean is the
        ...
        Gate 1's 0.169 was computed against the mean (s1_values.py, `ty[:, m].mean()`), so a median
...
raw value) and IQR / (p99 - p01).
    ...
    python s2_tokens.py --build     # wide value tables + panel.json + the dynamic-range score
    python s2_tokens.py --check     # GATE 2, four checks, writes reports/s2_masking.md
    python s2_tokens.py --check --refit    # ignore the checkpoint cache and refit every run
    python s2_tokens.py --check --quick    # tiny smoke run: proves the code path, scores nothing
"""
...
BATCH, LR   = 512, 3e-3
# 30 is a CEILING, not a schedule - early stopping on held-out slides picks the real number.
...
TEST_FRAC, VAL_FRAC = 0.20, 0.12          # SLIDE fractions, never cell fractions


def full_table(c):
    return os.path.join(VALUES, f'{c}_full.parquet')
...
def read_panel(cohorts=None):
    ...
    if not os.path.exists(p):
        raise FileNotFoundError(f"{p} missing - run `python pipeline2/s2_tokens.py --build` first.")
    o = json.load(open(p))
...
def do_build(cohorts, per):
    ...
    D.to_csv(os.path.join(WORK, 's2_dynrange.csv'), index=False)
    return D, pd.DataFrame(info)


# ----------------------------------------------------------------------------- data
def slide_split(cohort, images):
    """Split by SLIDE, never by cell.

    Held-out CELLS share their slide with training cells, so a model can score well by memorising
    slide-level quirks. Gate 1 hit the same issue and answered it by scoring leave-one-COHORT-out;
    here checks 1, 2 and 4 need a within-cohort holdout, so the holdout is by slide.
    """
    im = np.array(sorted(images))
    r = _rng('split', cohort).permutation(len(im))
    nt = max(1, int(round(TEST_FRAC * len(im))))
    nv = max(1, int(round(VAL_FRAC * len(im))))
    return set(im[r[nt + nv:]]), set(im[r[nt:nt + nv]]), set(im[r[:nt]])
...
def load_cohort(c, triples_c, tri2idx, n_vocab, arm, excluded, limit=None):
    ...
    tr_s, va_s, te_s = slide_split(c, np.unique(img))
    ...
    parts = dict(train=draw(tr_s, N_TRAIN, 'train'),
                 val=draw(va_s, VAL_CELLS, 'val'),
    ...
    return dict(cohort=c, tri=tri, triples=tl,
                idx=torch.from_numpy(np.ascontiguousarray(idx)).long(),
                present=None if present is None else torch.from_numpy(present),
                elig=torch.from_numpy(elig),
                U={k: torch.from_numpy(U[w]) for k, w in parts.items()},
...
def fit(data, train_cohorts, n_vocab, epochs=EPOCHS, patience=PATIENCE, seed=SEED, log=''):
    ...
    model = TokenModel(n_vocab, d=D_MODEL, blocks=BLOCKS, heads=HEADS)
    ...
    vhide = {c: make_hide(data[c]['n']['val'], data[c]['elig'], vgen) for c in usable}
    ...
    with torch.no_grad():
        for m, t in enumerate(d['tri']):
            ...
            for i in range(0, N, chunk):
                u = U[i:i + chunk]
                hide = torch.zeros(len(u), U.shape[1], dtype=torch.bool)
                hide[:, m] = True
...
def embed(model, d, chunk=1024):
    ...
    return torch.cat(out) if out else torch.zeros(0, D_MODEL)


def cohort_probe(Z, y, slides, seed=SEED, steps=400):
    """5-way logistic regression on the pooled embedding: can it tell which cohort a cell is from?
...
def cached(tag, refit, make):
    p = os.path.join(CKPT, f's2_{tag}.pt')
...
def holdout_run(tag, arm, train, per, tri2idx, V, excluded, refit, epochs, limit=None):
    """Train on training-slide cells of every training cohort, score on their held-out slides."""
    def make():
        ...
        # labels. Slide ids are prefixed with the cohort because they are only unique WITHIN a
        # cohort - Keren and Sorin both start numbering at 1.
        names = sorted(data)
        Z = torch.cat([embed(model, data[c]) for c in names])
        y = torch.cat([torch.full((data[c]['n']['test'],), names.index(c), dtype=torch.long)
                       for c in names])
        sl = np.concatenate([[f'{c}|{s}' for s in data[c]['img']['test']] for c in names])
        return dict(arm=arm, r2=R, info=info,
                    probe=cohort_probe(Z, y, sl), state=model.state_dict())
...
def loco_run(tag, arm, held, train, per, tri2idx, V, excluded, refit, epochs, limit=None):
    ...
    ferguson never appears here. It is the frozen holdout - one number, once, at Stage 7.
    """
    def make():
        ...
        return dict(arm=arm, held=held, r2=R, info=info, state=model.state_dict())
    return cached(tag, refit, make)
...
def write_report(out_path, ctx):
    ...
    A("Scored **on held-out slides**, not held-out cells. Held-out cells share their slide with "
      "training cells, so slide-level quirks can be memorised; a slide holdout removes that.\n")
    ...
    A(f"**PASS requires** every kept marker above 0, **and** a median kept-marker R2 of at least "
      f"{R2_MEDIAN_MIN}. Both declared in `pipeline2/panel/gate2_expect.csv` before the run.\n")
    ...
    A("The first Gate 2 run exposed a defect in this check, not in the model. The exclusion floor "
      ...
      "slides**. A marker expressed on some slides and not others clears the cohort-wide floor "
      "and still has a collapsed denominator exactly where it is scored.\n")
    ...
    A("This is open question **H3**, and it is what says whether Stage 2's complexity was earned. "
      ...
      "5 training-cohort folds. ferguson is not in any fold - it is the frozen holdout.\n")
    ...
    A("The held-out-Sorin fold made it visible. Sorin presents 12-17 tokens against the 39-57 the "
      ...
      "positive. Absent slots keep the set size fixed at 99 for every cohort, so the encoder never "
      "sees a set unlike anything in training.\n")
    ...
    A("\n### The cohort probe\n")
    A("A 5-way logistic regression on the pooled cell embedding, trained and scored on **different "
      "slides**. It is 5-way rather than the 6-way the design file wrote, because a probe is "
      "*trained* on these embeddings and ferguson is never trained on - so chance is 20%. It is "
    ...
    A(f"\nModel: `d_model={D_MODEL}`, {BLOCKS} blocks, {HEADS} heads, {int(MASK_FRAC*100)}% of "
      f"eligible markers hidden per cell, minimum 1. Early stopping on held-out slides of the "
      f"training cohorts, patience {PATIENCE}. Local CPU box, no CUDA.\n")
...
def do_check(cohorts, per, tri2idx, triples, genes, ncoh, D, refit=False, quick=False):
    ...
    if not quick:                       # record it where Stage 3 looks for it
        ...
        obj['stage2_arm_note'] = (
            ...
            f"{a_loco:.4f} vs arm B {b_loco:.4f} on the {len(core)} core markers. Stage 3 "
            f"warm-starts from work/ckpt/s2_arm{'B' if arm == 'absent' else 'A'}*.pt.")
    ...
    c3 = pd.DataFrame([
        dict(arm='Gate 1 V3 - concat MLP, 9 markers', markers=len(core),
             loco_r2_on_core=GATE1_V3_LOCO, note='already measured, reports/s1_normalisation.md'),
    ...
    c4_verdict = (
        ...
        ("Arm B's higher probe accuracy confirms the absent slots carry panel identity."
         ...
         "**The probe separates neither arm - both read 1.000.** So it cannot decide anything "
    ...
    runs = pd.DataFrame(
        [_row('arm A - measured markers only (holdout slides)', A_run),
         _row('arm B - all slots, [ABSENT] (holdout slides)', B_run),
    ...
    figp = os.path.join(FIGURES, 's2_rank_spread_vs_r2.png')
    ...
    ctx = dict(lines=[], D=D, core=core, genes=genes, V=V, per=per, verdict=verdict,
               ...
               c4=c4, c4_verdict=c4_verdict, runs=runs)
    path = write_report(os.path.join(REPORTS, 's2_masking.md'), ctx)
...
def do_armb_loco(held, cohorts, per, tri2idx, triples, ncoh, D, refit=False):
    ...
    B = loco_run(f'armb_loco_{held}', 'absent', held, train, per, tri2idx, V, excluded, refit,
                 EPOCHS)
    A = torch.load(os.path.join(CKPT, f's2_loco_full_{held}.pt'), weights_only=False)
    C = torch.load(os.path.join(CKPT, f's2_loco_core_{held}.pt'), weights_only=False)
...
def main():
    ...
    if '--build' in sys.argv or need:
        ...
        D, info = do_build(cohorts if '--build' in sys.argv else need, per)
        print(f"  wrote {write_panel(triples, per, genes, ncoh)}")
        if need and '--build' not in sys.argv:
            D = None            # partial build - reload the full table below

    if D is None:
        p = os.path.join(WORK, 's2_dynrange.csv')
        if not os.path.exists(p):
            sys.exit("no work/s2_dynrange.csv - run: python s2_tokens.py --build")
        D = pd.read_csv(p, keep_default_na=False)
        for col in ('tie_mass', 'robust_dispersion', 'var_ucoh', 'rank_spread'):
            if col not in D.columns:
                sys.exit(f"work/s2_dynrange.csv predates D-28 (no `{col}` column) - "
                         f"rerun: python s2_tokens.py --build")
    ...
    print(f"\n{'PASS' if passed else 'FAIL'} · {(time.time()-t0)/60:.1f} min · wrote {path}")
