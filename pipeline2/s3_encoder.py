...
space, so for the first time in the pipeline a cross-cohort classifier can actually be scored.
    ...
    python s3_encoder.py --lambda-sweep          # GATE 3
    python s3_encoder.py --lambda-sweep --quick  # smoke test: proves the code path, scores nothing
    python s3_encoder.py --lambda-sweep --lambdas 0,0.03,0.3   # a narrower grid
    python s3_encoder.py --lambda-sweep --refit  # ignore the checkpoint cache
    python s3_encoder.py --lambda-sweep --cpu    # force CPU even where a GPU exists
...
L2. The plan declared "LOCO L2 macro-F1 - this alone decides", but L2 comes from
_validation/hand_mapping_reference.csv, which covers 4 cohorts (CRC, Keren, UPMC, ferguson) - and
ferguson is the frozen holdout. So L2 exists for only 3 of the 5 TRAINING cohorts: Phillips and
Sorin have no L2 ground truth at all, and Sorin is the panel-mismatch stress test. The declared
metric is therefore not computable on this roster. The 25 clusters are the pipeline's actual
output space, cover all 106 labels and all 5 folds, and keep the decision off the hand mapping -
which D-4 says survives only as a test set. L2 is still reported, marked as 3-of-5 coverage.
...
from nn.encoder import CellEncoder, Heads, SlideProbe
...
def label_space():
    ...
    key = {(r.cohort, r.label): c2i[r.cluster] for r in lm.itertuples()}
    names = (lm.drop_duplicates('cluster').set_index('cluster').cluster_name
             .reindex(clusters).fillna('').to_dict())
    unreliable = sorted({c2i[r.cluster] for r in lm.itertuples() if int(r.unreliable) == 1})
    return dict(n=len(clusters), key=key, names=[names[c] for c in clusters],
                unreliable=set(unreliable), raw=lm)


def hand_l2():
    """The hand mapping, for the SECONDARY L2 column only. Never used to train (D-4)."""
    p = os.path.join(config.ROOT, '_validation', 'hand_mapping_reference.csv')
    if not os.path.exists(p):
        return None, []
    h = pd.read_csv(p, keep_default_na=False)
    lv = sorted(h.L2.unique())
    i = {v: k for k, v in enumerate(lv)}
    return {(r.cohort, r.native_label): i[r.L2] for r in h.itertuples()}, lv


# ----------------------------------------------------------------------------- data
def load_cohort(c, triples_c, tri2idx, n_vocab, arm, excluded, L, l2key):
    ...
    y2 = np.array([l2key.get((c, str(lab)), -1) for lab in v.native_label]) if l2key else \
        np.full(len(v), -1)
    ...
    # shrink for it.
    w_tr = np.flatnonzero(np.isin(img, list(tr_s)))
    ...
    parts = dict(train=train_ix, sprobe=probe_ix, val=draw(va_s, N_TRAIN // 4, 'val'),
    ...
    pat = np.array([f'{c}|{p}' for p in v.patient_id])
    return dict(cohort=c, pat={k: pat[w] for k, w in parts.items()},
                ...
                y2={k: torch.from_numpy(y2[w]).long().to(DEV) for k, w in parts.items()},
...
def l2_score(d, yp, n_l2, unreliable):
    """Secondary L2 column (D-31). Never decides anything.

    The classifier predicts CLUSTERS, so a predicted cluster has to be read as an L2 class. The
    map is a majority vote taken inside the HELD-OUT cohort's own hand mapping - which reads the
    hand mapping as a TEST SET exactly as D-4 permits, and never as training signal. Coverage is
    3 of 5 folds: Phillips and Sorin have no L2 truth at all.
    """
    y2 = d['y2']['test'].cpu().numpy()
    yc = d['y']['test'].cpu().numpy()
    # The stroma waiver applies here too (D-32), or this column would not be comparable with the
    # f1_core it sits beside in the report.
    ok = (y2 >= 0) & ~np.isin(yc, list(unreliable))
    if int(ok.sum()) < 50:
        return float('nan'), 0
    cl2l2 = {int(cl): int(np.bincount(y2[ok][yc[ok] == cl]).argmax())
             for cl in np.unique(yc[ok])}
    p2 = np.array([cl2l2.get(int(p), -1) for p in yp[ok]])
    f, _ = macro_f1(y2[ok], p2, n_l2)
    return f, int(ok.sum())
...
# ----------------------------------------------------------------------------- runs
def loco_run(tag, lam, held, train, per, tri2idx, V, L, l2key, l2names, arm, warm, refit, epochs,
             nested=False, deep_slide=False):
    """Train on the other training cohorts at this lambda, score on the held-out one."""
    p = os.path.join(CKPT, f's3_{tag}.pt')
    ...
    data = {c: load_cohort(c, per[c], tri2idx, V, arm, None, L, l2key) for c in train}
    ...
    enc, heads, info = fit(data, rest, lam, V, L['n'], sidx, warm=warm, epochs=epochs,
                           deep_slide=deep_slide)
    ...
    f1_l2, n_l2_cells = l2_score(data[held], yp, len(l2names), L['unreliable'])
    ...
    r = dict(lam=lam, held=held, f1_core=f1_core, f1_all=f1_all, f1_waived=f1_waived,
             ...
             f1_l2=f1_l2, n_l2_cells=n_l2_cells,
             ...
             info=info, seconds=round(time.time() - t0, 1),
...
def assemble(res, L, lams):
    ...
    d = pd.DataFrame([{k: r[k] for k in
                       ('lam', 'held', 'f1_core', 'f1_all', 'f1_waived', 'f1_majority',
                        'f1_random', 'f1_l2', 'n_l2_cells', 'fresh_bits', 'fresh_acc',
...
def write_report(path, d, per_lam, checks, ship, verdict, L, l2names, arm, dev, mins,
                 complete=True):
    ...
    partial = '' if complete else (
        ...
        f"{len(LAMBDAS)} x 5. The verdict line below is arithmetic on what ran, not a gate "
...
Arm **{arm}** from Gate 2. Label space: {L['n']} Stage 1b clusters, {len(L['unreliable'])} flagged
...
{md(d[['lam', 'held', 'f1_core', 'f1_l2', 'n_l2_cells']].round(4))}
...
def available():
    ...
    Deliberately NOT `s2.built()`, which tests for work/raw/{c}.parquet. Those are acquisition
...
def main():
    ...
    if '--folds' in sys.argv:
        ...
        print(f"  --folds: holding out only {folds} (still training on the other 4)")
    if quick:
    ...
    warm = None
    wp = os.path.join(CKPT, 's2_armA.pt' if arm == 'set' else 's2_armB.pt')
    if '--no-warm' not in sys.argv and os.path.exists(wp):
        warm = torch.load(wp, weights_only=False)['state']
        print(f"warm-starting the token layer from {os.path.basename(wp)}")
    ...
    for lam in lams:
        print(f"\n  lambda = {lam}")
        for h in folds:
            res.append(loco_run(f'{q}{tagpfx}lam{lam}_{h}', lam, h, train, per, tri2idx, V, L,
                                l2key, l2names, arm, warm, '--refit' in sys.argv, epochs,
                                nested=nested, deep_slide=deep_slide))
    ...
    torch.save(light, os.path.join(CKPT, f's3_{q}{tagpfx}sweep.pt'))
    print(f"\nwrote {os.path.join(CKPT, f's3_{q}{tagpfx}sweep.pt')}")
    ...
    complete = (sorted(lams) == sorted(LAMBDAS) and len(folds) == len(train) == 5)
    ...
    name = (f's3b_{tagpfx}encoder{"_PARTIAL" if not complete else ""}.md' if tagpfx else
            's3_encoder.md' if complete else 's3_encoder_PARTIAL.md')
    p = write_report(os.path.join(REPORTS, name), d, per_lam, checks, ship, verdict,
                     L, l2names, arm, DEV, (time.time() - t0) / 60, complete)
