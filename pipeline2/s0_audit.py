...
a hand-written blacklist here.
    ...
    python s0_audit.py            # audit whatever is built
    python s0_audit.py --build    # (re)build every cohort first
"""
import os, sys, json
import numpy as np
import pandas as pd
...
def main(do_build=False):
    ...
    fig.suptitle('GATE 0 - one representative slide per cohort (coloured by native label)',
                 fontsize=10)
    fig.tight_layout()
    figp = os.path.join(FIGURES, 's0_tissue_check.png')
    ...
    path = os.path.join(REPORTS, 's0_audit.md')
