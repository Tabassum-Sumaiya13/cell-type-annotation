...
output is byte-identical. That matters because Kaggle notebooks have internet off by default.
    ...
    python s0b_markers.py             # resolve (uses cache, calls API for misses)
    python s0b_markers.py --offline   # fail rather than call the network; proves reproducibility
"""
...
def gate(R):
    ...
    path = os.path.join(REPORTS, 's0b_markers.md')
