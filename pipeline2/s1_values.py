...
independent information, and prevalence is kept by `u_coh` itself.
    ...
    python s1_values.py              # build work/values/*.parquet + slide statistics
    python s1_values.py --bakeoff    # the five GATE 1 checks + reports/s1_normalisation.md
"""
...
def build(cohort, triples, n_sub=N_SUB, rng=None):
    ...
    # module docstring in nn/marker_encoder.py.
...
def do_bakeoff(cohorts, triples, genes, refit=False):
    ...
    M, cache = len(triples), os.path.join(WORK, 's1_folds.pt')
...
def write_report(cohorts, triples, genes, B, build_rows):
    ...
    figp = os.path.join(FIGURES, 's1_no_cliffs.png')
    ...
    A("Assertions declared in advance in `pipeline2/panel/gate1_expect.csv`: a cell whose "
    ...
    path = os.path.join(REPORTS, 's1_normalisation.md')
